...
import com.urk17cs290.mediaplayer.music.songdata.Playlist;
...
public class PlaylistUtils {
   ...
   */
  public static void newPlaylist(Context c, String fromWhere, String name, List<Song> songsToAdd) {
    ...
    // Okay its strange that sometimes newPlaylistUri is null, It means playlist is already there,
    // So i am deleting the playlist and creating new
    ...
    String[] playlistColumns = {PLAYLIST_ID, PLAYLIST_NAME};
    ...
    // Now, to it's data
    Uri songUri =
        Uri.withAppendedPath(newPlaylistUri, MediaStore.Audio.Playlists.Members.CONTENT_DIRECTORY);
    int songOrder = 1;
    ...
    for (Song song : songsToAdd) newPlaylist.add(song.getId());
  ...
  public static List<String> getPlaylistNames() {
    ...
    for (Playlist playlist : Main.data.playlists) names.add(playlist.getName());
  ...
  public static List<Song> getSongsByPlaylist(String playlistName) {
    ...
    if (songIDs != null)
      for (Long songID : songIDs) currentSongs.add(SongUtils.getSongById(songID));
  ...
  public static String getPlayListId(String selectedplaylist) {
    ...
    if (getPlaylistNames().contains(selectedplaylist)) {
      for (Playlist playlist : Main.data.playlists) {
        if (selectedplaylist.equals(playlist.getName())) id = String.valueOf(playlist.getID());
      }
    }
    return id;
  }
  ...
  /* Renaming  Playlist */
  public static void renamePlaylist(Context context, String newplaylist, long playlistId) {
    ContentResolver resolver = context.getContentResolver();
    ContentValues values = new ContentValues();
    String where = _ID + " =? ";
    String[] whereVal = {Long.toString(playlistId)};
    values.put(NAME, newplaylist);
  ...
  public static void deletePlaylistTrack(Context context, String name, long audioId) {
    final Uri uri =
        MediaStore.Audio.Playlists.Members.getContentUri(
            "external", Long.parseLong(getPlayListId(name)));
    final ContentResolver resolver = context.getContentResolver();
    resolver.delete(
        uri,
        MediaStore.Audio.Playlists.Members.AUDIO_ID + " = ? ",
        new String[] {Long.toString(audioId)});
    for (Playlist p : Main.data.playlists) {
  ...
  public static void addSongsToplaylist(Context c, String name, List<Song> songsToAdd) {
