package com.urk17cs290.mediaplayer.music.transition;
...
public class AnimatedVectorDrawableCompat {
    ...
    public static void removeListener(AnimatedVectorDrawable drawable, Object callback) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (callback instanceof Animatable2.AnimationCallback) {
                drawable.unregisterAnimationCallback((Animatable2.AnimationCallback) callback);
            } else {
                throw new IllegalArgumentException("Callback parameter must implement " + Animatable2.AnimationCallback.class.getName());
            }
        }
